...
export default function ChannelPartnerList() {
  ...
  const [selectedPartnerForInvoice, setSelectedPartnerForInvoice] = useState<any>(null);
  const [editingPartnerId, setEditingPartnerId] = useState<string | null>(null);
  const { data: editingPartner, isLoading: isEditingPartnerLoading, error: editingPartnerError } = useGetMerchantQuery(editingPartnerId || "", { skip: !editingPartnerId });
  if (isEditingPartnerLoading) { /* noop */ }
  ...
  useEffect(() => {
    if (location.pathname.endsWith("/new")) { setEditingPartnerId(null); openModal(); }
  }, [location.pathname]);

  const closeAndNavigate = () => { closeModal(); navigate("/channel-partners"); };

  const handleCreateOrUpdateSuccess = () => {
    const wasEdit = !!editingPartner;
    closeAndNavigate();
    setEditingPartnerId(null);
    dispatch(addNotification({ variant: 'success', title: wasEdit ? 'Channel partner updated' : 'Channel partner created', message: wasEdit ? 'Channel partner updated successfully' : 'Channel partner created successfully' }));
  };
  ...
  useEffect(() => {
    if (editingPartnerError) {
      const err: any = editingPartnerError as any;
      dispatch(addNotification({ variant: 'error', title: 'Failed to load channel partner', message: err?.data?.message || err?.message || 'Failed to fetch details' }));
      setEditingPartnerId(null);
      closeAndNavigate();
    }
  }, [editingPartnerError]);
  ...
  const downloadSampleCsv = () => {
    const sample = "firstName,lastName,email,mobile,businessName,customerType,modeOfPayment,commissionPercentage";
    const blob = new Blob([sample], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "channel-partners-sample.csv";
    a.click();
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2 items-center">
            ...
            <Link to="/channel-partners/new" className="inline-flex h-9 items-center justify-center rounded-full bg-indigo-600 px-4 text-xs font-medium text-white hover:bg-indigo-500">Add Channel Partner</Link>
          </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left">
                ...
                <th className="text-sm font-medium text-slate-900 dark:text-slate-50">Commission %</th>
            ...
            <tbody>
              ...
              {!isLoading && partners.length === 0 && (<tr><td colSpan={8}>No channel partners</td></tr>)}
              {partners.map((m: any) => (
                <tr key={m.id} className="border-t">
                  <td className="py-3 text-sm text-slate-800 dark:text-slate-100"><Link to={`/channel-partners/${m.id}`} className="text-brand-500 hover:underline">{m.firstName} {m.lastName}</Link></td>
                  ...
                  <td className="text-sm text-slate-700 dark:text-slate-200">{m.commissionPercentage != null ? `${m.commissionPercentage}%` : '-'}</td>
                  ...
                  <td className="flex gap-2">
                    <button className="text-sm text-brand-500" onClick={() => { setEditingPartnerId(String(m.id)); openModal(); }}>Edit</button>
                    <button className="text-sm text-blue-500" onClick={() => { setSelectedPartnerForInvoice(m); openInvoiceModal(); }}>Invoice</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <Modal isOpen={isOpen} onClose={() => { closeAndNavigate(); setEditingPartnerId(null); }} className="max-w-md lg:max-w-lg m-4">
        {isEditingPartnerLoading ? (
          <div className="p-4">Loading channel partner details...</div>
        ) : (
          <ChannelPartnerForm partner={editingPartnerId ? (editingPartner || null) : null}
            onSuccess={handleCreateOrUpdateSuccess}
            onCancel={() => { closeAndNavigate(); setEditingPartnerId(null); }}
      ...
      <InvoiceModal 
        ...
        partnerOrCustomer={selectedPartnerForInvoice} 
