...
export default function CustomerForm({ onSuccess, onError, onCancel, customer }: { onSuccess?: () => void; onError?: (msg: string) => void; onCancel?: () => void; customer?: Merchant | null }) {
  ...
  const validate = () => {
    ...
    if (!customer && (!form.password || form.password.length < 8)) e.password = "Password min 8 chars";
    setErrors(e);
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    try {
      ...
      if (!customer) {
        ...
        await updateMerchant({ id: customer.id, body: payload }).unwrap();
      }
      onSuccess?.();
    } catch (err: any) {
  ...
  return (
    <div className="max-h-[70vh] overflow-y-auto rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      <h3 className="text-lg font-medium mb-3 text-slate-900 dark:text-slate-50">{customer ? "Edit Customer" : "Add Customer"}</h3>
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="flex justify-end gap-2">
          ...
          <button type="submit" disabled={creating || updating} className="inline-flex h-9 items-center justify-center rounded-full bg-indigo-600 px-4 text-sm font-medium text-white hover:bg-indigo-500">{creating || updating ? (customer ? "Updating..." : "Creating...") : customer ? "Update" : "Create"}</button>
        </div>
      </form>
    </div>
  );
